...
function TooltipP({ active, payload }: any) {
  ...
  return (
    <div className="bg-[var(--c-card)] border border-[var(--c-border)] rounded-xl p-3 text-xs">
      ...
      <p className="text-[var(--c-text3)]">{formatCLP(d.monto)}</p>
      <p className="text-[var(--c-text4)]">{d.cantidad} deuda{d.cantidad !== 1 ? "s" : ""}</p>
    </div>
  )
}
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="text-center">
            ...
            <p className="text-[9px] text-[var(--c-text4)]">Total</p>
          </div>
        </div>
      </div>
        ...
        {datos.map((d, i) => {
          ...
          return (
            <div key={d.tipo} className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-[var(--c-text3)] flex-1 truncate">{d.tipo}</span>
              <span className="text-[10px] text-[var(--c-text4)]">{pct}%</span>
              <span className="text-[10px] font-medium text-[var(--c-text2)]">{formatCLP(d.monto)}</span>
            </div>
          )
        })}
      </div>
